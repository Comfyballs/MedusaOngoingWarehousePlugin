...
- It asks Ongoing only for orders **above the same done threshold** whose **status changed in the last ~26 hours** (Ongoing's `orderStatusChangedTimeFrom` filter), so the query stays small no matter how much finished history the warehouse holds. The window is wider than 24 hours on purpose — the sweep can only start on a poll tick, and the extra margin means no change falls between two sweeps.
...
- One order that fails to sync is logged and skipped, and the sweep is still recorded as run — a single bad order can't turn the daily pass into an every-tick one. It gets another chance the next day. If the Ongoing query itself fails, no sweep is recorded and the next poll tick retries it.
