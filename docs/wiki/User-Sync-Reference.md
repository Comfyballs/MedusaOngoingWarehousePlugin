...
| Daily done-order sweep | Same status-poll job, gated to once per 24 h per integration | Re-sync finished orders | Re-asks Ongoing for orders **above the done threshold** whose status changed in the past ~26 h and runs each through the normal refresh/shipment/delivery path — the catch-net for late changes the poll no longer sees | Per-order errors logged and skipped; the sweep still counts as run. A failed query is retried on the next tick |
...
### The daily safety-check sweep — what still catches a late change

Freezing an order at the threshold would otherwise lose anything Ongoing does to it afterwards: a pickup order advancing `451` (Klar til henting) → `500` (Hentet), a `475` (Retur), an annulment, corrected tracking. Once a day per integration, the poll job runs a second, low-frequency pass to catch exactly those:

- It asks Ongoing only for orders **above the same done threshold** whose **status changed since the last sweep** (Ongoing's `orderStatusChangedTimeFrom` filter), so the query stays small no matter how much finished history the warehouse holds. The window is measured from the previous sweep plus a margin — never a fixed 24 hours — so a sweep delayed by an Ongoing outage, a restart, or a long `status_poll_interval` widens its own window to cover the whole gap instead of skipping over it. An integration that has not swept for more than 30 days gives up on the older history rather than requesting all of it at once.
- Every match goes through the **same** path as a normal poll: status refresh, then shipment or delivery if the code calls for it. All of it is idempotent, so re-examining an order that has not really changed does nothing.
- It reuses the integration's Ongoing connection and its rate limit — the sweep does not open a second parallel channel to Ongoing.
- Rows that reached a **terminal** sync state (`delivered`, `cancelled`) are left alone; they are finished on the Medusa side too.
- One order that fails to sync is logged and skipped, and the sweep is still recorded as run — a single bad order can't turn the daily pass into an every-tick one. That reconciliation is **dropped**, not retried: unless the order changes again in Ongoing, the next sweep's window starts after it. The log line says so explicitly, so a recurring entry here is worth investigating. If the Ongoing query itself fails, no sweep is recorded and the next poll tick retries the whole pass.

The sweep is a reconciliation net with up to a day of latency, not a replacement for the webhook. If you need a `451` → `500` pickup confirmation in Medusa within minutes rather than by the next day, configure the [[webhook|User-Setup-Guide]] or raise `defaultDoneStatusThreshold` above `451` so the 15-minute poll keeps following the order.
