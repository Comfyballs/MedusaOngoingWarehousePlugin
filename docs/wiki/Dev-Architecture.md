...
| `last_done_sweep_at` | dateTime, nullable | When the daily done-order safety sweep last completed (bead `t36`). Advanced only on a successful sweep; null/older than 24 h makes the next poll tick run one. |
| `sync_lock_until` | dateTime, nullable | Advisory lock TTL for the status-poll job. Held for at least 15 minutes on a tick that also runs the daily done-order sweep. |
...
Bookkeeping columns (`last_stock_sync_at`, `last_status_poll_at`, `last_done_sweep_at`) record the last tick.
...
- `status-poll.ts`: per enabled integration, acquires its own `"status_poll"` lock, calls `getOrdersByStatus(100, 999)`, refreshes each matched non-terminal row's `latest_status_code/text` via `refreshOngoingOrderStatusWorkflow` (bead `o6c`; the same workflow the webhook status refresh uses), then resolves each order's status to a stage (`resolveShipmentStage`, bead `18m`): a `shipped` stage runs `syncOngoingShipmentWorkflow`, a `delivered` stage runs `syncOngoingDeliveryWorkflow`. `shipped` rows stay in scope (only `delivered`/`cancelled` are terminal) so the `450` → `500` pickup transition is caught. Rows stamped `done_synced_at` are skipped by this pass (bead `8rg`) and revisited by the **daily done-order sweep** below. The lock/cadence bookkeeping (`acquireSyncLock`/`releaseSyncLock`/`updateOngoingIntegrations` timestamp stamp) stays a direct module-service call — tangled with the advisory-lock `finally` lifecycle rather than a per-order data mutation, so it's left as-is (a documented, lower-priority `arch-workflow-required` deviation).
- `status-poll.ts` — **daily done-order sweep** (bead `t36`): the same job, second pass, gated per integration on `last_done_sweep_at` (24 h). Once a row is stamped `done_synced_at` the 15-minute pass never looks at it again, so a late Ongoing change would be invisible — `451` → `500` (pickup collected), a `475` return, an annulment, corrected tracking. The sweep calls `getOrdersByStatus(doneThreshold + 1, 1000, changedSince)`, where `changedSince` maps to the spec's **`orderStatusChangedTimeFrom`** ("only return orders whose status has changed after this time" — live-verified: accepted, narrows the set, and carries no 24 h age limit, unlike `stockInfoChangedFrom`). The lookback is **derived from the measured gap since `last_done_sweep_at`** (`resolveSweepLookbackMs`: `elapsed + 2 h`, floored at 26 h, capped at 30 days), never from the nominal interval — the sweep is *due* at 24 h but only *runs* on a poll tick that reaches it, so an outage, a restart, a paused worker or a large `status_poll_interval` can stretch the real gap arbitrarily. A fixed window would leave everything older than it unexamined by any sweep, ever. Matches (non-terminal rows only, including done-stamped ones) run through the **same** per-order apply path as the poll — `refresh → shipment/delivery → mark-done` — all of which are idempotent, so a redundant re-sync is a no-op. Orders already applied by the poll pass in the same tick are skipped. The sweep also runs when the **poll pass threw**: `pollAndApply` captures that error, runs the sweep, then rethrows, because a row that can never be stamped done (an orphan, a deleted fulfillment) throws on every tick and would otherwise keep the safety net switched off forever. Per-order sweep failures are caught and logged, and the sweep is still stamped — one unsyncable order must not turn a daily pass into a per-tick one; that reconciliation is **dropped**, which the log line states and follow-up bead `t36.1` tracks. A failure of the *query* leaves `last_done_sweep_at` unstamped, so the next tick retries. It uses the integration's shared `OngoingClient`, hence the same per-credential-key throttle.
