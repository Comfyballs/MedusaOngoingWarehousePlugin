...
- `status-poll.ts` — **daily done-order sweep** (bead `t36`): the same job, second pass, gated per integration on `last_done_sweep_at` (24 h). Once a row is stamped `done_synced_at` the 15-minute pass never looks at it again, so a late Ongoing change would be invisible — `451` → `500` (pickup collected), a `475` return, an annulment, corrected tracking. The sweep calls `getOrdersByStatus(doneThreshold + 1, 1000, changedSince)`, where `changedSince` is `now − 26 h` and maps to the spec's **`orderStatusChangedTimeFrom`** ("only return orders whose status has changed after this time" — live-verified: accepted, narrows the set, and carries no 24 h age limit, unlike `stockInfoChangedFrom`). The lookback is deliberately wider than the 24 h interval because the sweep can only fire on a poll tick. Matches (non-terminal rows only, including done-stamped ones) run through the **same** per-order apply path as the poll — `refresh → shipment/delivery → mark-done` — all of which are idempotent, so a redundant re-sync is a no-op. Orders already applied by the poll pass in the same tick are skipped. Per-order failures are caught and logged, and the sweep is still stamped: one unsyncable order must not turn a daily pass into a per-tick one. A failure of the *query* leaves `last_done_sweep_at` unstamped, so the next tick retries. It uses the integration's shared `OngoingClient`, hence the same per-credential-key throttle.
