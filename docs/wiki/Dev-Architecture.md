...
`push-return-order-to-ongoing` (the outbound return push) and `sync-ongoing-return-status` (this inbound handling) both have no dedicated `OngoingOrderSync`-style ledger row for returns — success/failure/status is tracked only via `ONGOING_EVENTS` emits (`RETURN_ORDER_PUSHED`/`RETURN_ORDER_PUSH_FAILED`/`RETURN_STATUS_RECEIVED`) plus application logs, no migration.

**Decision: keep it that way for now** (events + logs), rather than add an `OngoingReturnSync` model in this pass. Rationale:

- **Schema is premature**: a useful return-ledger row needs fields for the still-unverified pieces above — the confirmed `PUT /returnOrders` response id field (bead `2a6`) and real "received" status codes. Designing the columns now risks a second migration once those are confirmed.
- **Ambiguous keying today**: the inbound webhook has no `returnOrderNumber` to key a ledger row on, so an inbound-status ledger write today could only be scoped to the *original* order (already possible via `OngoingOrderSync`, see above), not the specific return — the same disambiguation problem that blocks a Medusa-side mutation blocks a precise ledger write too.
- **Lower risk**: this repo's own precedent (`push-return-order.ts`'s comment) already chose events-plus-logs for the outbound push for the same "no migration this pass" reason; keeping the inbound side consistent avoids a half-migrated feature (outbound events-only, inbound with a row).
- **Not free of cost**: this does mean returns have no admin-dashboard-queryable state today (unlike outbound orders' `OngoingOrderSync` rows) — an ops user can only see return activity in logs/emitted events, not the "Failed & pending syncs" DataTable. That gap is real and worth closing once the schema questions above are resolved.

Follow-up: `MedusaOngoingWarehousePlugin-2a6` (live-sandbox verification) is a prerequisite for designing an `OngoingReturnSync` ledger with confidence; a dedicated ledger bead should be filed once that lands, rather than schema-guessing now.
