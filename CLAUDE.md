...
- **`src/jobs/`** — `status-poll.ts`, `stock-sync.ts` (every 15 minutes), `retry-failed-syncs.ts` (every 5 minutes): poll Ongoing order status, reconcile `stocked_quantity` (delta or full sweep), and sweep failed sync rows with exponential backoff/dead-lettering.
...
- **`src/api/`** — `admin/ongoing/*` routes (integration CRUD, per-order repush/sync, syncs dashboard/bulk-retry/orphan-repair, test-connection, credential-keys) under Medusa's default admin auth; `ongoing/webhooks/[credentialKey]/route.ts` is the Ongoing status-webhook receiver with custom static-secret auth (never core admin/store auth).
